...
namespace AdventOfCode2018.Days {
    public class Day07 {
        ...
        public static async Task Solve() {
            ...
            Console.WriteLine(PartTwo(steps, 5, 60));
            Console.ReadKey();
        }

        private static string PartOne(IList<(char, char)> steps) {
            var path = steps
                .Select(x => x.Item1)
                .Except(steps
                    .Select(x => x.Item2))
            ...
            var distinct = steps
                .Select(x => new List<char> { x.Item1, x.Item2 })
                .SelectMany(x => x)
                .Distinct()
            ...
            return Enumerable.Range(0, distinct.Count)
                .Aggregate(path, (a, b) => a + distinct
                        ...
                        .Where(x => !steps.Where(y => y.Item2 == x).Select(y => y.Item1).Except(a.ToCharArray()).Any())
                        .OrderBy(y => y)
                        .FirstOrDefault());
        }

        private static int PartTwo(IList<(char, char)> steps, int workersMax, int secondsBase) {
            var workers = steps
                .Select(x => x.Item1)
                .Except(steps
                    .Select(x => x.Item2))
            ...
            while (done.Count < count) {
                ...
                var newItems = distinct
                    .Except(done)
                    .Except(workers.Select(x => x.Step))
                    .Where(x => !steps.Where(y => y.Item2 == x).Select(y => y.Item1).Except(done).Any())
                    .OrderBy(y => y)
                    .Take(workersMax - workers.Count)
                    .Select(x => (x, 0));

                workers.AddRange(newItems);
                minutes++;
            }
